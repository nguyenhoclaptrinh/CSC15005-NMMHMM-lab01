...
BigInt BigInt::operator+(const BigInt &other) const {
    BigInt r; r.data.assign(max(data.size(), other.data.size()), 0);
    uint64_t carry = 0;
    ...
    if (carry) r.data.push_back(uint32_t(carry));
    return r.normalize();
}

BigInt BigInt::operator-(const BigInt &other) const {
    // Giả sử *this >= other
    BigInt r; r.data.assign(data.size(), 0);
    int64_t borrow = 0;
    for (size_t i = 0; i < data.size(); ++i) {
        int64_t a = data[i];
        int64_t b = (i < other.data.size() ? other.data[i] : 0);
        int64_t d = a - b - borrow;
        if (d < 0) { d += int64_t(BASE); borrow = 1; } else borrow = 0;
...
BigInt BigInt::operator*(const BigInt &other) const {
    ...
    BigInt r; r.data.assign(na + nb, 0);
    for (size_t i = 0; i < na; ++i) {
        ...
        if (carry) r.data[i + nb] += uint32_t(carry);
    }
    return r.normalize();
}
...
static int msb_index(const BigInt &x) {
    for (int wi = int(x.data.size()) - 1; wi >= 0; --wi) {
        ...
        if (w) {
            for (int b = 31; b >= 0; --b) if ((w >> b) & 1u) return wi * 32 + b;
        }
    }
    return -1;
}
static int get_bit(const BigInt &x, int bit) {
    if (bit < 0) return 0;
    int wi = bit / 32; int bi = bit % 32;
    if (wi >= (int)x.data.size()) return 0;
...
static void set_bit(BigInt &x, int bit) {
    int wi = bit / 32; int bi = bit % 32;
    if (wi >= (int)x.data.size()) x.data.resize(wi + 1, 0);
    x.data[wi] |= (1u << bi);
}
...
BigInt BigInt::operator/(const BigInt &other) const {
    ...
    bool zero = true; for (auto w : other.data) if (w) { zero = false; break; }
    if (zero) throw runtime_error("divide by zero");
    if (*this < other) return BigInt(0);
    BigInt q(0), r(0);
    int nbit = msb_index(*this);
    for (int b = nbit; b >= 0; --b) {
        shl1(r);
        if (get_bit(*this, b)) r.data[0] |= 1u;
        if (!(r < other)) { r = r - other; set_bit(q, b); }
    }
    return q.normalize();
}

BigInt BigInt::operator%(const BigInt &mod) const {
    bool zero = true; for (auto w : mod.data) if (w) { zero = false; break; }
    if (zero) throw runtime_error("mod by zero");
...
std::string BigInt::to_decimal() const {
    ...
    while (!(tmp.data.size() == 1 && tmp.data[0] == 0)) {
        ...
        for (int b = nbit; b >= 0; --b) {
            shl1(r);
            if (get_bit(tmp, b)) r.data[0] |= 1u;
            if (!(r < ten)) { r = r - ten; set_bit(q, b); }
        }
...
std::istream &operator>>(std::istream &in, BigInt &val) {
    std::string s; in >> s; val = BigInt(s); return in;
}

std::ostream &operator<<(std::ostream &out, const BigInt &val) {
    // Xuất hex đơn giản
    BigInt v = val; v.normalize();
